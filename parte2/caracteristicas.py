...
def construir_caracteristicas_mano(fila: pd.Series) -> pd.Series:
    ...
    # Straight (incluye caso A alto: {1,10,11,12,13})
    es_straight = 1 if (uniq_ranks == 5 and (max(ranks_sorted) - min(ranks_sorted) == 4)) else 0
    if not es_straight and set(ranks_sorted) == {1, 10, 11, 12, 13}:
        es_straight = 1

    # Otras derivadas útiles
    hay_full_house = 1 if (hay_trio == 1 and n_pares == 1) else 0
    suit_cnt_max = max(Counter(suits).values())
    ...
    base = {
        ...
        "hay_trio": hay_trio,
        "hay_poker": hay_poker,
        "hay_full_house": hay_full_house,
        "suit_cnt_max": suit_cnt_max,
        "es_flush": es_flush,
